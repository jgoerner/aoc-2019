 ...
 * Then, treat the fuel amount you just calculated as the input mass and repeat the process, continuing until a
 ...
 */
fun massToFuel(mass: Int, simple: Boolean = true): Int = if (simple) {
    (floor(mass / 3.0) - 2).toInt()
} else {
    if (mass >= MINIMAL_MASS_FOR_FUEL_INCREMENT) { massToFuel(mass) + massToFuel(massToFuel(mass), false) }
    else { 0 }
}
